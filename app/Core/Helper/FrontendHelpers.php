...
class FrontendHelpers
{
    ...
    public static function renderImage($url, $alt = false, $title = false, $class = false) {
        ...
        ob_start();
        ?>
        <img src="<?php echo $url ?>" alt="<?php echo $alt ? $alt : ( isset($pathinfo['filename']) ? $pathinfo['filename'] : "" ) ?>" title="<?php echo $title ? $title : (isset($pathinfo['filename']) ? $pathinfo['filename'] : "" ) ?>" <?php echo $class ? 'class="'. $class . '"': false ?>/>
        <?php
